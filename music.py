...
class SpotifyService:
	...
	def test_connection(self):
		print(f'Accessing Spority with {self._access_code}')
...
class SpotifyServiceBuilder:
	...
	def __call__(self, spotify_client_key, spotify_client_secret, **_ignored):
		print(f"I've just entered the __call__() funtion with spotify_client_key = {spotify_client_key} and spotify_client_secret = {spotify_client_secret}")
...
class MusicServiceProvider(object_factory.ObjectFactory):
	def get(self, servic_id, **kwargs):
		return self.create(servic_id, **kwargs)
